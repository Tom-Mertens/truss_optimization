...
class Vizualization():
    ...
    def visualizeAreas(self, elementNodes, nodes, AreaOpt, supports, force, scale, stress, linewidth, maxiter):
        ...
        figcaption('Topology Opt. of a 2D truss Structure at {} iterations.'.format(maxiter),
                   label="fig:{}".format(maxiter))
        ...
        plt.axis('off')
        plt.annotate('force', xy=(nodes[force, 0], nodes[force, 1]),
                     xytext=(nodes[force, 0], nodes[force, 1] + len_ele / 2.0),
...
class FEA():
    ...
    def structure2(self, Numx, Numy, Coord=10.0):
        ...
        if Numx == 0 or Numy == 0:
            ...
            nodes = np.flip(np.array(np.meshgrid(y, x, indexing='ij')).T.reshape(-1, 2),
                            1)  # We'll always have two dims in this problem
            ...
            while Ny <= Numy - 1:
                ...
                while Nx <= Numx - 1:
                    ...
                    if Ny == 0:
                        GeneralElem += [
                            [coordinate_grid[:, Nx, Ny], coordinate_grid[:, Nx + 1, Ny]]]  # elem 5 in the borders
                    Nx += 1
                Ny += 1
    ...
    def formStiffness2Dtruss(self, A, GDof, elementNodes, nodes, indices, E):
        ...
        for e in elementNodes:
            elemDof = np.array(
                [indices[count, 0] * 2, indices[count, 0] * 2 + 1, indices[count, 1] * 2, indices[count, 1] * 2 + 1])
            count += 1
